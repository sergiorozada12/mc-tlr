...
class SCPD:
    ...
    def fit(self, chain, Q_emp):
        ...
        obs_pairs = torch.where(Q_emp.flatten() != 0)[0] if self.sampling_type == "entry" else None
        R_emp = 0.5 * (Q_emp.sum(tuple(range(D, 2*D))) + Q_emp.sum(tuple(range(D)))) if self.sampling_type == "trajectory" else None
        chain_transitions = torch.hstack([torch.stack(chain[1:]), torch.stack(chain[:-1])]) if self.sampling_type == "trajectory" else None
        ...
        num_mttkrps = int(self.num_itrs / mttkrp + .5)
        B_init = self.B
            ...
            for inn_itr in range(mttkrp):
                ...
                if d < DD:
                    Gd = self._factor_compute_grad(
                        d, chain, Q_emp, Qds_upd, l, Q_est,
                        obs_pairs, R_emp, chain_transitions
                    )
                    factor_grad_mags[d] += torch.linalg.norm(Gd.reshape(-1), 1)
                    Qd_last = Qds[d].clone()
                    Q_dot, alpha_factor = self._grad_step(Qds[d], Gd, alpha_factor, factor_grad_mags[d])
                    Qds[d] = self._proj_factor(Q_dot)
                    Qds_upd[d] = Qds[d] + (1 - t_last) / t * (Qds[d] - Qd_last) if self.acceleration else Qds[d].clone()
                else:
                    gl = self._weight_compute_grad(
                        chain, Q_emp, Qds, l_upd,
                        obs_pairs, R_emp, chain_transitions
                    )
                    weight_grad_mag += torch.linalg.norm(gl.reshape(-1), 1)
                    l_last = l.clone()
                    l_dot, alpha_weight = self._grad_step(l, gl, alpha_weight, weight_grad_mag)
                    l = self._proj_weight(l_dot)
                    l_upd = l + (1 - t_last) / t * (l - l_last) if self.acceleration else l.clone()

                Q_est = cp_to_tensor((l, Qds))
                diff = norml1_err(Q_est, Q_last)
                cost = kld_err(Q_emp, Q_est)
                var = np.var(costs[tot_itr - self.slide_window:tot_itr]) if tot_itr >= self.slide_window else 0.
                ...
                if (out_itr == 0 and inn_itr == 0) or (toc - tic > 2):
                    print(f"MTTKRP: {out_itr+1}/{num_mttkrps} | Cost: {cost:.3e} | Diff: {diff:.1e} | Var: {var:.1e}")
                    tic = perf_counter()
        ...
        Q_est = cp_to_tensor((l, Qds))
        P_est = Qten2Pten(Q_est)
        return dict(mc_est=MarkovChainTensor(P_est), diffs=diffs, costs=costs, Qds=Qds, l=l)
    ...
    def _proj_factor(self, Qd):
        ...
        proj = [proj_bounded_simplex((Qd[:, k] - self.qmin) / (self.qmax - self.qmin), s) for k in range(Qd.shape[1])]
    ...
    def _proj_weight(self, l):
        return self.qmin + (self.qmax - self.qmin) * torch.FloatTensor(
            proj_bounded_simplex((l - self.qmin) / (self.qmax - self.qmin), (1 - len(l) * self.qmin) / (self.qmax - self.qmin))
        )

    def _weight_compute_grad(self, chain, Q_emp, Qds, l, obs_pairs, R_emp, chain_transitions):
        DD = Q_emp.ndim
        D = DD // 2
        IIs = torch.tensor(Q_emp.shape)
        Is = IIs[:D]
        ...
        if self.sampling_type == "fiber":
            ...
            if self.gamma_weight > 0:
                ...
                gl_erg = self.gamma_weight * ((H0_erg - H1_erg).T @ (H0_erg - H1_erg)) @ l / B_erg
                gl += gl_erg

        elif self.sampling_type == "entry":
            ...
            gls = [-(Q_emp[*step] * II) * hls[i] / (hls[i] @ l + self.eps) for i, step in enumerate(steps)]
            ...
            if self.gamma_weight > 0:
                ...
                gls_erg = [self.gamma_weight * (II * I) * torch.outer(hs0[i] - hs1[i], hs0[i] - hs1[i]) @ l for i in range(B_erg)]
                gl_erg = torch.stack(gls_erg).sum(0) / B_erg
                gl += gl_erg

        elif self.sampling_type == "trajectory":
            if R_emp is None:
            ...
            if self.gamma_weight > 0:
                ...
                hs0 = [torch.stack([Qds[d][step[d % D]] for d in range(D)]).prod(0) for step in steps_erg]
                hs1 = [torch.stack([Qds[d][step[d % D]] for d in range(D, DD)]).prod(0) for step in steps_erg]
                gls_erg = [self.gamma_weight / R_emp[*step] * torch.outer(hs0[i] - hs1[i], hs0[i] - hs1[i]) @ l for i, step in enumerate(steps_erg)]
                gl_erg = torch.stack(gls_erg).sum(0) / B_erg
                gl += gl_erg

        return gl

    def _factor_compute_grad(self, d, chain, Q_emp, Qds, l, Q_est, obs_pairs, R_emp, chain_transitions):
        DD = Q_emp.ndim
        D = DD // 2
        IIs = torch.tensor(Q_emp.shape)
        Is = IIs[:D]
        ...
        if self.sampling_type == "fiber":
            ...
            if self.gamma_factor > 0:
                ...
                Gd_erg = self.gamma_factor * (Q0_erg + Q1_erg).T @ H_erg @ torch.diag(l) / B_erg
                Gd += Gd_erg
            ...
            gds = [-(Q_emp[*step] * II) * hds[i] * l / (Q_est[*step] + self.eps) for i, step in enumerate(steps)]
            ...
            if self.gamma_factor > 0:
                ...
                gds_erg = [self.gamma_factor * (II * I) * (Q0[*step] + Q1[*step]) * hds_erg[i] * l for i, step in enumerate(steps_erg)]
            ...
            hds = [torch.stack([Qds[j][step[j]] for j in range(DD) if j != d]).prod(0) for step in steps]
            gds = [-hds[i] * l / (Q_est[*step] + self.eps) for i, step in enumerate(steps)]
            Gds = [torch.outer(torch.eye(IIs[d])[step[d]], gds[i]) for i, step in enumerate(steps)]
            Gd = torch.stack(Gds).sum(0) / B
            ...
            if self.gamma_factor > 0:
                ...
                Q1 = cp_to_tensor((l, Qds[D:])) * -(2 * (d < D) - 1)
                hds_erg = [torch.stack([Qds[j][step[j % D]] for j in np.arange(D) + D * (d >= D) if j != d]).prod(0) for step in steps_erg]
                gds_erg = [self.gamma_factor / R_emp[*step] * (Q0[*step] + Q1[*step]) * hds_erg[i] * l for i, step in enumerate(steps_erg)]
                Gds_erg = [torch.outer(torch.eye(IIs[d])[step[d % D]], gds_erg[i]) for i, step in enumerate(steps_erg)]
                Gd_erg = torch.stack(Gds_erg).sum(0) / B
                Gd += Gd_erg
